...
"""
'''
based on GTCRN from https://github.com/Xiaobin-Rong/gtcrn/blob/main/gtcrn.py
hybrid GTCRN using dual mic and iva feature
40.2MMacs, 24.39 K params
'''
...
class FeatureSelection(nn.Module):
    def __int__(self, ):
        super().__init__()
    def IVA_SN_LPS_feature(self, feat):
        """
        spec: (ch, feat, time)
        """
        feat = feat.transpose((1, 2, 0))  # (F,T,ch)
        feat = AuxIVA(feat, n_src=2, n_iter=100, distribution="laplace", return_filter=False)  # (F,T,ch)
        feat_0 = feat[..., 0]  # (F,T) complex
        feat_1 = feat[..., 1]  # (F,T) complex
        feat_0_lps = np.log(np.square(np.abs(feat_0)) + eps) # (F,T)
        feat_1_lps = np.log(np.square(np.abs(feat_1)) + eps)
        feat = np.stack([ feat_0_lps, feat_1_lps], axis=-1) # (F,T,2) lps feature

        return feat
    
class H_GTCRN(nn.Module):
    def __init__(self):
        super().__init__()
        self.erb_y = ERB(65, 64)
        self.erb_iva = ERB(65, 64)
        self.sfe_y = SFE(3, 1)
        self.sfe_iva = SFE(3, 1)
    ...
    def forward(self, feat):
        """
        spec: (B, F, T, 6) #channel: real0, imag0, real1, imag1, iva s lps, iva n lps
        """
        spec_ref = feat[..., 0:2]  # (B, F, T, 2) real0, imag0
        feat_y = feat[..., 0:4].permute(0,3,2,1)  # (B,4,T,257)
        feat_iva = feat[..., 4:6].permute(0,3,2,1)
        
        feat_y = self.erb_y.bm(feat_y)  # (B,4,T,129)

        feat_y = self.sfe_y(feat_y)     # (B,12,T,129)

        feat_iva = self.erb_iva.bm(feat_iva)  # (B,2,T,129)

        feat_iva = self.sfe_iva(feat_iva)     # (B,6,T,129)

        feat = torch.cat([feat_y, feat_iva], dim=1) # (B,18,T,129)
...
if __name__ == "__main__":
    ...
    flops, params = get_model_complexity_info(model, (257, 65, 6), as_strings=True,
                                           print_per_layer_stat=True, verbose=True)
    print(flops, params)
    in_2_ch_noisy = torch.randn(2, 16000)
    #y_feat [ch, feat, time, ri]
    y_feat = torch.stft(in_2_ch_noisy, 512, 256, 512, torch.hann_window(512).pow(0.5), return_complex=False)
    fs = FeatureSelection()
    y_feat_complex = np.complex64(y_feat[..., 0] + 1j * y_feat[..., 1]) # [ch, feat, time]

    iva_feat = fs.IVA_SN_LPS_feature(y_feat_complex) # [F, T, 2]
    iva_feat = torch.from_numpy(iva_feat)

    y0_feat = y_feat[0,...] #[F, T, 2]
    y1_feat = y_feat[1,...]
    feat = torch.concat([y0_feat, y1_feat, iva_feat], dim=2) #[ F, T, 2]
    feat = feat.unsqueeze(0) #add batch size [B, F, T, CH]
    y = model(feat)
    print(y.shape)  # should be (B, F, T, 2)
'''
    """causality check"""
